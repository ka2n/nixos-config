...
Your workspace is `external-docs/` — all research artifacts (cloned repos, saved pages, documentation) go here so they persist across sessions and are available to other agents. Think of `external-docs/` as your filing cabinet; anything worth downloading is worth filing.
...
5. **Repository Cloning**: When you need to read repository source code:
   ```bash
   # Standard clone command — just replace OWNER/REPO
   git clone --depth 1 https://github.com/OWNER/REPO external-docs/cloned-repos/REPO
   ```
